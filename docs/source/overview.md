...
BLCI is meant to be used a version control system. In fact it relies on [Git](https://git-scm.com/) for version control so using it as a version control system would be redundant. If a user performs *git commits* without using the BLCI's `console.py` undefined behavior may occur.
